extension String {
  ...
  public init?(assemblySanitized sanitized: String) {
    ...
    while i != sanitized.endIndex {
      guard
        let (delta, j) = Base64UnsignedInteger.decode(from: sanitized[i...]),
        let (point, k) = Base64UnsignedInteger.decode(from: sanitized[j...])
      else { return nil }
  ...
  public var assemblySanitized: String {
    ...
    for (offset, point) in unicodeScalars.enumerated() {
      ...
      Base64UnsignedInteger(delta).write(to: &instructions)
      Base64UnsignedInteger(point.value).write(to: &instructions)
